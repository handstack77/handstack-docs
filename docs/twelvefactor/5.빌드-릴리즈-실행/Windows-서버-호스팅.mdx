...
<configuration>
    ...
    <location path="." inheritInChildApplications="false">
        <system.webServer>
            ...
            <aspNetCore processPath="ack.exe" arguments="" hostingModel="outofprocess" stdoutLogEnabled="false" stdoutLogFile="..\log\stdout"></aspNetCore>
        </system.webServer>
    </location>
</configuration>
```

`ack`는 self-contained가 아닌 apphost(`ack.exe`)로 게시되므로 `processPath`는 `dotnet`이 아니라 `ack.exe`를 가리키고, `hostingModel`은 `outofprocess`를 사용합니다. 이 설정은 저장소의 `1.WebHost/ack/web.config`를 기준으로 합니다.
