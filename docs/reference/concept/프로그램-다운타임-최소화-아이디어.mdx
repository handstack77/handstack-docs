...
> 실습으로 익히려면 학습 트랙의 [12. 모니터링과 운영 인수인계](/docs/tutorial/모니터링과-운영인수인계/)를 참고하세요.
...
```bash
# 모놀리식 구성으로 프로그램 실행
ack --modules=wwwroot,transact,dbclient,graphclient,function,command,prompter,repository,logger,checkup,forwarder --port=8421

# 마이크로 서비스 구성으로 프로그램 실행
ack --modules=wwwroot --port=8001
ack --modules=repository --port=8002
ack --modules=logger --port=8003
ack --modules=transact,dbclient,graphclient,function,command,prompter --port=8004
