...
이것은 프로젝트별 스킬의 예시입니다. 당신의 프로젝트를 위한 템플릿으로 사용하세요.

실제 프로덕션 애플리케이션인 [Zenith](https://zenith.chat) - AI 기반 고객 발굴 플랫폼을 기반으로 합니다.

---

## 사용 시점

이 스킬이 설계된 특정 프로젝트에서 작업할 때 참조하세요. 프로젝트 스킬은 다음을 포함합니다:
- 아키텍처 개요
- 파일 구조
- 코드 패턴
- 테스팅 요구사항
...
- **AI**: 도구 호출(Tool calling) 및 구조화된 출력이 포함된 Claude API
- **배포**: Google Cloud Run
...
2. **불변성** - 객체나 배열을 직접 수정하지 말 것
3. **TDD** - 구현 전에 테스트 작성
4. **80% 커버리지** 최소 요구사항
5. **다수의 작은 파일** - 보통 200-400 라인, 최대 800 라인
