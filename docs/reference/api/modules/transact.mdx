...
- `wwwroot`의 개발용 테스트 계정 자동 로그인(`/wwwroot/api/dev-account/sign-in`)이 발급한 요청은 `IsDevAutoSignIn` claim으로 표시됩니다. `TransactionController`/`WorkflowController`는 이 claim을 감지하면 경고 로그를 남기고, `AppSettings:IsEnabledDevAutoSignIn`이 `false`이거나 `RunningEnvironment`가 `D`가 아닌데도 claim이 존재하면(위조 의심) 요청을 거부합니다.
