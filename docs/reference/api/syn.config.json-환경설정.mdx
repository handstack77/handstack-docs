...
transact 모듈의 API 서버의 검색 기능을 사용할 때 검색 서버의 URL을 설정합니다. 기본값은 빈 문자열("")입니다. 검색 서버는 화면에서 서버의 자원을 검색할 때 사용합니다.
...
화면에서 사용할 도메인 스크립트 경로와 공통으로 적용할 스타일/스크립트/컨트롤 자산 목록을 설정합니다. Application, Definition의 정보를 주의 깊게 설정 해야 합니다.

```json
{
    "Application": {
        "LoaderPath": "/js/syn.domain.js", // 화면에서 사용하는 기본 도메인 스크립트 경로입니다.
        "CodeHelpID": "" // 코드도움을 위한 기본 거래 ID입니다.
    },
    "Definition": {
        "BindingAction": "Append", // Append: 기본 자산 목록에 추가, Replace: 기본 자산 목록을 대체합니다.
        "Styles": [], // 추가로 로드할 CSS 파일 경로 목록입니다.
        "Scripts": [], // 추가로 로드할 JavaScript 파일 경로 목록입니다.
        "Controls": [] // 추가로 로드할 UI 컨트롤 스크립트 경로 목록입니다.
    }
}
```

> 태넌트 앱의 `app.environment.json`에서는 `EnvironmentSetting`의 내용을 최상위 객체로 두고 `Application`, `Cookie`, `Header`, `Definition`을 함께 사용할 수 있습니다. `Cookie`, `Header`는 요청 시 추가로 전송할 쿠키/헤더 값을 지정합니다.