...
`syn.$w`는 단순 유틸리티 묶음이 아니라 화면 초기화와 거래 실행의 중심입니다.

1. 브라우저가 `syn.js`를 로드합니다.
2. `synConfig` 또는 `synConfigName`으로 `syn.Config`가 병합됩니다.
3. `DOMContentLoaded` 시점에 `syn.$w.contentLoaded()`가 실행됩니다.
4. 현재 화면의 `moduleScript` 또는 URL 파일명을 기준으로 `syn.$w.pageScript`가 결정됩니다.
5. 화면 모듈의 `config`, `prop`, `transaction`, `hook`, `event`, `method`가 초기화됩니다.
6. `hook.pageLoad()`와 컨트롤 이벤트 바인딩이 실행됩니다.

화면 업무 코드는 이 흐름을 전제로 작성해야 합니다.
...
`transactionAction`은 현재 화면 모듈의 `transaction` 설정에서 `actionID`를 찾고, 입력 데이터 수집과 출력 바인딩을 처리합니다. 일반 화면 버튼 이벤트에서는 `transactionDirect`보다 `transactionAction`을 우선 사용합니다.
...
syn.$w.transactionDirect({
    ...
    functionID: 'LD01',
    inputObjects: {
        SearchKeyword: 'handstack'
    }
}, function(error, result) {
    if (error) {
        syn.$l.eventLog('$sample.transactionDirect', error, 'Error');
        return;
    }
...
`transactionDirect`는 화면의 `transaction` 설정을 거치지 않고 거래 객체를 직접 만들어야 할 때 사용합니다. 내부적으로는 `executeTransaction` 흐름을 사용하므로 `syn.Config.Transaction`, `DomainAPIServer`, 인증 헤더, 압축 설정의 영향을 받습니다.
...
#### syn.$w.apiHttp(url)
HandStack 화면에서 자주 쓰는 API 호출 빌더입니다. `send()` 호출 시 GET/POST/FormData 요청을 처리하고, `syn.Config.IsClientCaching` 설정에 따라 cache busting을 적용합니다.

```javascript
const list = await syn.$w.apiHttp('/sample/data.json').send();

const saved = await syn.$w.apiHttp('/sample/save').send({
    userID: 'tester',
    useYN: 'Y'
});
...
const uploaded = await syn.$w.apiHttp('/sample/upload').send(formData);
```

#### syn.$w.startIntersection(id, placeholder, loadMore, options)
IntersectionObserver 기반 무한 스크롤을 시작합니다. 목록 하단 placeholder가 보이면 `loadMore(done)` 콜백을 실행합니다.

```javascript
syn.$w.startIntersection('order-list', 'divLoadMore', async (done) => {
    await $this.method.loadNextPage();
    done($this.prop.hasMoreData);
});

syn.$w.stopIntersection('order-list');
```
