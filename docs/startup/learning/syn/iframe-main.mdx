...
- 설명: 다른 창(주로 `<iframe>`의 `contentWindow`)과 `postMessage` 기반 양방향 채널을 생성합니다.
- 매개변수
...
- 설명: `scope`가 `channelID`와 일치하는 연결된 채널을 `$network.connections`에서 찾습니다.
- 매개변수
...
- 설명: `rooms.connect()`가 반환한 채널 객체를 통해 상대 창에 등록된(`bind`) 메서드를 호출하고 결과를 비동기로 받습니다.
- 매개변수
