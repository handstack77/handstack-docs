...
- `[syn-i18n]` 스캔은 페이지 로드 완료 이후(`syn.$w.readyAsync()` resolve 시점) 수행되므로, 페이지 진입 직후 바로 `getControl()`을 호출하면 아직 등록되지 않았을 수 있습니다. 준비 완료를 기다리려면 `await syn.$w.readyAsync()`를 사용하십시오.
