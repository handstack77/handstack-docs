...
```javascript
// 공통 리소스 추가와 조회
syn.$res.add('saveComplete', '정상적으로 저장되었습니다.');
const message = syn.$res.get('saveComplete');

// locale 파일을 읽고 화면 번역 다시 적용
await syn.$res.setLocale('ko-KR');
syn.$res.translatePage();

// 화면 모듈에서 locale별 문구 보강
let $sample = {
    hook: {
        pageResource(localeID) {
            syn.$res.add('customTitle', localeID === 'ko-KR' ? '고객 관리' : 'Customer');
        }
    }
};
```
...
```javascript
// Reportify Excel 템플릿 기반 PDF 출력
const reportID = syn.$w.pageScript.substring(1);
const excelUrl = syn.$p.getDocumentTemplateUrl(reportID);

await syn.$p.renderPrint(reportID, {
    excelUrl,
    workData: {
        FormData0: {
            CustomerName: '홍길동',
...
`syn.$p`는 현재 Reportify/Excel 템플릿 기반 출력 흐름에 맞춰 사용합니다. 임의 HTML을 직접 PDF로 변환하는 범용 `printPage`, `printHtml` API가 아닙니다.
...
- [HandStack 화면 HTML 작성 지침](/docs/information/resource/HandStack-화면-HTML-작성-지침)
- [HandStack 화면 JavaScript 작성 지침](/docs/information/resource/HandStack-화면-JavaScript-작성-지침)
