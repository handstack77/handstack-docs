...
#### canShare(data) / share(data)
Web Share API 지원 여부를 확인하고 브라우저의 공유 UI를 호출합니다. 지원되지 않는 환경에서는 내부적으로 클립보드 복사 같은 대체 흐름을 사용할 수 있습니다.
...
Performance Timeline API에서 navigation, resource, measure 항목을 조회합니다.

```javascript
const navigationEntries = syn.$b.getPerformanceEntries({ type: 'navigation' });
const transactionEntries = syn.$b.getPerformanceEntries({
    type: 'resource',
    name: '/transact/api/transaction/execute'
});
```

#### markPerformance(markName) / measurePerformance(measureName, startMark, endMark)
화면의 특정 처리 구간을 측정할 때 사용합니다.

```javascript
syn.$b.markPerformance('search-start');
await syn.$w.transactionAction('LD01');
syn.$b.markPerformance('search-end');

const entries = syn.$b.measurePerformance('search-duration', 'search-start', 'search-end');
