...
여러분의 기업 내 애플리케이션의 환경 설정과 개발 소스는 프론트엔드와 백엔드 모두 `C:\home\handstack\contracts` 단일 디렉토리내에서 관리됩니다. 이를 통해 Visual Studio, Eclipse, Visual Studio Code, Notepad++ 등등 프로젝트 디렉토리 기반 소스 편집 기능을 제공하는 개발 도구로 `C:\home\handstack\contracts`를 지정하여 개발 가능합니다.
