...
|약어|컴포넌트명|설명|
|---|---|---|
|syn.uicontrols.$checkbox|CheckBox|브라우저마다 다르게 표시되는 체크박스를 일관되게 표현합니다.|
|syn.uicontrols.$codepicker|CodePicker|코드도움 팝업 기능을 제공합니다.|
|syn.uicontrols.$colorpicker|ColorPicker|색상 팔레트를 제공합니다.|
|syn.uicontrols.$contextmenu|ContextMenu|오른쪽 마우스 버튼 기능을 활용하여 컨텍스트 메뉴를 제공합니다.|
|syn.uicontrols.$data|DataSource|단일 또는 여러 데이터 건의 데이터 소스 객체 기능을 제공합니다.|
|syn.uicontrols.$dateperiodpicker|DatePeriodPicker|시작 및 종료 기간 날짜 선택 기능을 제공합니다.|
|syn.uicontrols.$datepicker|DatePicker|날짜 선택 기능을 제공합니다.|
|syn.uicontrols.$multiselect|DropDownCheckList|여러 개의 항목을 선택 할 수 있는 콤보박스를 제공합니다.|
|syn.uicontrols.$select|DropDownList|단일 항목을 선택 할 수 있는 콤보박스를 제공합니다.|
|syn.uicontrols.$element|Element|HTML 항목에서 getValue, setValue 기능을 제공합니다.|
|syn.uicontrols.$fileclient|FileClient|파일 업로드/다운로드 기능을 제공합니다.|
|syn.uicontrols.$list|GridList|데이터 조회에 특화된 기능을 제공합니다.|
|syn.uicontrols.$guide|Guide|화면 내 표시되는 도움말 기능을 제공합니다.|
|syn.uicontrols.$htmleditor|HtmlEditor|파일 업로드/다운로드 기능이 통합된 HTML 편집기 기능을 제공합니다.|
|syn.uicontrols.$organization|OrganizationView|조직도 데이터를 직관적으로 편집하는 기능을 제공합니다.|
|syn.uicontrols.$echarts|ECharts|객체/객체 배열을 다양한 차트로 표현하고 선택한 원본 행을 거래 값으로 제공합니다.|
|syn.uicontrols.$radio|RadioButton|브라우저마다 다르게 표시되는 라디오버튼을 일관되게 표현합니다.|
|syn.uicontrols.$sourceeditor|SourceEditor|소스 편집기에 특화된 에디터 기능을 제공합니다.|
|syn.uicontrols.$textarea|TextArea|여러 텍스트 줄을 관리하기 위한 추가 기능을 제공합니다.|
|syn.uicontrols.$textbox|TextBox|단일 텍스트을 관리하기 위한 추가 기능을 제공합니다.|
|syn.uicontrols.$button|Button|버튼을 관리하기 위한 추가 기능을 제공합니다.|
|syn.uicontrols.$tree|TreeView|재귀적으로 트리 구조를 표현하는 기능을 제공합니다.|
|syn.uicontrols.$grid|WebGrid|대량 편집에 가능한 그리드 컴포넌트 기능을 제공합니다. (Handsontable 유료 라이선스 필요)|
|syn.uicontrols.$auigrid|AUIGrid|대량 편집에 가능한 그리드 컴포넌트 기능을 제공합니다. (AUISoft 유료 라이선스 필요)|

위의 UI 컴포넌트은 다양한 상용/오픈소스 기반 라이브러리를 활용되었으며 지속적으로 업데이트 됩니다.

> `%HANDSTACK_HOME%\modules\wwwroot\wwwroot\lib` 디렉토리에서 기본 제공되는 컴포넌트의 기반인 오픈소스와 상용 라이브러리를 확인 할 수 있습니다.
...
Highcharts와 Chart.js 기반 데이터 시각화 기능을 제공합니다. 두 컨트롤 모두 객체/객체 배열을 `setValue`로 받고 선택한 point의 원본 행을 Row/List 거래 형식으로 반환합니다.
...
#### 사용법
```html
<syn_chart id="chtHigh" style="width:450px;height:320px" syn-options="{option:{chart:{type:'column'}}}"></syn_chart>
<syn_chartjs id="chtJs" style="width:450px;height:320px" syn-options="{type:'bar'}"></syn_chartjs>
```

상세 데이터 어댑터, 모듈, 선택 API는 [Chart](uicontrols/chart) 문서를 확인하세요.

## ECharts

객체 또는 객체 배열을 ECharts `dataset.source`로 연결하고 선택한 point의 원본 행을 Row/List 거래 형식으로 반환합니다.
...
#### 사용법
```html
<syn_echarts id="chtSales" style="width:100%;height:360px" syn-options="{
  option: {
