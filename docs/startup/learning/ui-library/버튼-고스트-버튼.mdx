...
평상시에는 배경과 테두리가 전혀 없다가 마우스를 올리거나(hover) 포커스가 갔을 때만 배경색이 옅게 드러나는, **가장 절제된 형태의 버튼**입니다. `btn-ghost-*` 클래스로 적용하며, 툴바나 카드 액션처럼 시각적 무게를 최소화해야 하는 위치에 적합합니다. [아웃라인 버튼](버튼-아웃라인-버튼)보다 한 단계 더 배경에 가까운 스타일이라고 생각하면 됩니다.
...
<UiLibraryPreview src="버튼-고스트-버튼" />
