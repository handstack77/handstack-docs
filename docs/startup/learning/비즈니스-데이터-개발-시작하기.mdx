...
* Method: Post
* Url: http://localhost:8080/transact/api/transaction/execute
...
위와 같은 요청/응답 데이터 구조는 HandStack 기반 화면이나 프로그램, 각 서비스 간에 자동으로 만들고 전달 되는 전문 프로토콜입니다. 일반적으로 개발자가 위와 같은 데이터를 작성하지 않습니다.
