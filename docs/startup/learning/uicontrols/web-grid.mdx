...
과거에는 이 폴더에 `AUIGrid`/`AUIPivot`/`OpenGrid`까지 함께 있었지만, 각각 별도 디렉토리(및 별도 문서)로 분리되었습니다.
...
- 다른 엔진: [AUIGrid](aui-grid), [AUIPivot](aui-pivot), [OpenGrid](open-grid)
...
| `exportFile(elID, options)` / `exportAsString(elID, options)` / `importFile(elID, callback)` | CSV/엑셀 내보내기·가져오기. `importFile`은 `callback` 생략 시 `Promise` 반환 |
| `validateColumns(elID, columns, callback)` / `validateRows(elID, rows, callback)` | 유효성 검사. `callback` 생략 시 결과(boolean)로 resolve되는 `Promise` 반환 |
...
- CRUD 상태 플래그(`Flag`): `$grid`/`$auigrid`/`$auipivot`/`$opengrid`는 행 데이터에 `Flag` 컬럼(R=조회/C=생성/U=수정/D=삭제/S=고정)을 유지하며, `getValue` 또는 `getUpdateItems`가 이 플래그를 기준으로 변경된 행만 추려서 서버 전송용 배열을 만듭니다.
...
- `syn.uicontrols.$xxx.controlLoad(elID, setting)`: 프레임워크가 `<syn_grid>`/`<syn_auigrid>`/`<syn_opengrid>` 태그를 파싱해 자동으로 호출합니다. 자동주입을 지원하지 않는 엔진은 페이지의 로딩 훅에서 직접 호출하며, 원래 마크업 요소는 `{elID}_hidden`으로 이름이 바뀌고 `syn-options`에 최종 설정이 JSON으로 다시 기록됩니다.
