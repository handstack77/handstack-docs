...
`/uicontrols/FileClient/example/` 경로의 예제를 아래 iframe에서 바로 확인할 수 있습니다.

### basic.html

<iframe
  src="/uicontrols/FileClient/example/basic.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="FileClient basic 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/FileClient/example/methods.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="FileClient methods 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/FileClient/example/settings.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="FileClient settings 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/FileClient/example/attachments.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="FileClient attachments 실전 예제"
  allow="fullscreen">
</iframe>
...
|---|---|
| 컨트롤 | `FileClient` |
| 전역 별칭 | `syn.uicontrols.$fileclient` |
| 버전 | `v2025.9.16` |
| 소스 | `uicontrols/FileClient/FileClient.js` |
| 스타일 | `uicontrols/FileClient/FileClient.css` |
...
| `fileDownload` | `fileDownload(elIDOrOptions)` | 파일을 다운로드한다. 문자열(`elID`)을 넘기면 그 컨트롤의 현재 값(`ItemID`)을 다운로드하고, `객체 설정` 객체를 넘기면 임의의 파일을 다운로드(다중 업로드에서 특정 항목만 받을 때 사용) |
...
> `callback` 인자를 받는 메서드(`getItem`, `getItems`, `deleteItem`, `deleteItems`, `updateDependencyID`, `updateFileName`, `uploadBlob`, `uploadDataUri`, `uploadBlobUri`, `fileUpload`)는 **`callback`을 생략하면 동일한 결과로 resolve되는 `Promise`를 반환**합니다(`await` 사용 가능). 요청 실패·검증 실패 시 `null` 로 resolve 됩니다.
