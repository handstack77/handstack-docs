...
3. TinyMCE 엔진 자체(`/lib/tinymce/tinymce.min.js`)는 `syn.$w.loadScriptAsync`로 페이지에서 최초 1회만 로드되며(로드 Promise는 `$htmleditor.tinymceReady`에 보관), 로드가 끝나기 전에 여러 개의 `<syn_htmleditor>`가 있으면 내부 대기열(`editorPendings`)에 쌓였다가 로드 완료 Promise가 resolve되면 순서대로 초기화됩니다(이전의 25ms `setInterval` 폴링은 제거됨).
...
> TinyMCE는 비동기로 로드/초기화됩니다. 페이지 로드 직후 곧바로 `getValue`/`setValue`를 호출하기보다는, 버튼 클릭 등 사용자 이벤트 시점에 호출하거나 `{elID}_documentReady` 이벤트(아래 API.md 참고)가 발생한 뒤에 호출하는 것이 안전합니다. 코드에서 준비 완료를 기다려야 하면 `await syn.uicontrols.$htmleditor.ready('edtComment')`를 사용합니다.
...
<iframe
  src="/uicontrols/HtmlEditor/example/document.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="HtmlEditor document 실전 예제"
  allow="fullscreen">
</iframe>
