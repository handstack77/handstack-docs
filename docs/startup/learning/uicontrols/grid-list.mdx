...
WebGrid(`$grid`, jqGrid 기반)와의 차이가 가장 헷갈리기 쉬운 부분입니다.
...
`/uicontrols/GridList/example/` 경로의 예제를 아래 iframe에서 바로 확인할 수 있습니다.

### basic.html

<iframe
  src="/uicontrols/GridList/example/basic.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="GridList basic 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/GridList/example/checkbox.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="GridList checkbox 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/GridList/example/events.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="GridList events 실전 예제"
  allow="fullscreen">
</iframe>
...
|---|---|
| 컨트롤 | `GridList` |
| 전역 별칭 | `syn.uicontrols.$list` |
| 버전 | `v2025.3.1` |
| 소스 | `uicontrols/GridList/GridList.js` |
| 스타일 | `uicontrols/GridList/GridList.css` |
...
| 옵션 | 타입 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `columns` | array | (필수, 기본값 없음) | 컬럼 정의 배열. 각 항목은 `객체 설정` 형태이며, `title`은 헤더 텍스트로 사용된 뒤 내부적으로 배열에서 제거됩니다(DataTables 컬럼 옵션에는 `title`을 남기지 않음). |
| `checkbox` | boolean | `false` | `true`면 첫 번째 컬럼에 체크박스 선택 컬럼을 추가하고, `select`를 `객체 설정`로 강제 설정합니다. 체크박스 모드에서는 `sScrollY`, `fnDrawCallback` 옵션이 자동으로 제거됩니다. |
...
| `select` | boolean \| object | `true` | 행 선택 사용 여부/방식. `checkbox: true`이면 내부적으로 `객체 설정`로 덮어써집니다. |
...
| `getControl(elID)` | 내부 상태 객체(`객체 설정`)를 반환합니다. `table`은 DataTables 1.10+ API 객체, `list`는 레거시(`fnXXX`) API 객체입니다. 체크박스 선택값은 `getControl(elID).table.column(0).checkboxes.selected().toArray()`로 조회합니다. |
