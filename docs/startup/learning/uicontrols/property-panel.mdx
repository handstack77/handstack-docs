...
PropertyPanel는 자바스크립트 객체 하나를 "속성명 / 값" 두 칸짜리 표 형태로 보여주고, 값의 타입에 맞는 입력 위젯(텍스트, 숫자, 체크박스, 색상, 드롭다운, JSON 편집 등)을 자동으로 만들어 주는 컨트롤입니다. Visual Studio나 여러 디자인 툴에서 흔히 보는 "속성(Properties)" 패널과 같은 역할을 합니다.
...
이미 화면 레이아웃이 고정되어 있고 입력 항목 종류가 소수로 정해져 있다면, TextBox/DropDownList/CheckBox 같은 개별 컨트롤을 직접 배치하는 편이 더 간단합니다. PropertyPanel는 "객체의 속성 구성 자체가 가변적이거나 많을 때" 진가를 발휘합니다.
...
```html
<syn_propertypanel id="ppBasic" syn-options="{
    data: {
        Name: '홍길동',
        Age: 32,
        UseYN: true,
...
```html
<syn_propertypanel id="ppOptions" syn-options="{
    sort: true,
    isCollapsible: true,
    data: {
        Width: 240,
...
```html
<syn_propertypanel id="ppEvents" syn-options="{
...
```js
var value = syn.uicontrols.$propertypanel.getValue('ppBasic');
syn.uicontrols.$propertypanel.setValue('ppBasic', { Name: '김철수', Age: 28, UseYN: false, FavoriteColor: '#dc2626' });
syn.uicontrols.$propertypanel.clear('ppBasic');
```

## 예제 실행하기

`example/` 폴더에 4가지 예제가 있습니다. 로컬 서버 실행 후 브라우저에서 아래 경로로 접근해서 확인하세요 (예: `/uicontrols/PropertyPanel/example/basic.html`).
...
- `binding.html` / `binding.js` — 속성 객체 전체를 `syn.$bind` Proxy 모델과 양방향으로 연결하는 예제입니다.

## 더 알아보기

- 옵션, `meta`/`customTypes` 세부 규칙, 메서드, 콜백 시그니처의 전체 목록은 아래 API 참조 섹션을 참고하세요.
- 실제 소스: `wwwroot/uicontrols/PropertyPanel/PropertyPanel.js`, `PropertyPanel.css`
- 스타일 커스터마이징: `PropertyPanel.css`의 `.syn-propertypanel`, `.ppTable`, `.ppGroupRow`, `.ppRow`, `.ppInput`, `.ppInvalid` 클래스를 통해 색상/레이아웃을 재정의할 수 있습니다.

## 실전 예제 페이지

`/uicontrols/PropertyPanel/example/` 경로의 예제를 아래 iframe에서 바로 확인할 수 있습니다.

### basic.html

<iframe
  src="/uicontrols/PropertyPanel/example/basic.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="PropertyPanel basic 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/PropertyPanel/example/options.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="PropertyPanel options 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/PropertyPanel/example/events.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="PropertyPanel events 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/PropertyPanel/example/binding.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="PropertyPanel binding 실전 예제"
  allow="fullscreen">
</iframe>
...
|---|---|
| 컨트롤 | `PropertyPanel` |
| 전역 별칭 | `syn.uicontrols.$propertypanel` |
| 소스 | `uicontrols/PropertyPanel/PropertyPanel.js` |
| 스타일 | `uicontrols/PropertyPanel/PropertyPanel.css` |

## API 참조

싱글턴 객체: `syn.uicontrols.$propertypanel`
소스 파일: `wwwroot/uicontrols/PropertyPanel/PropertyPanel.js`, `wwwroot/uicontrols/PropertyPanel/PropertyPanel.css`

## 마크업

PropertyPanel는 네이티브 태그가 아니라 `<syn_propertypanel>` 커스텀 태그로 사용합니다.

```html
<syn_propertypanel id="ppSample" syn-options="{
...
- `id`는 페이지 내에서 유일해야 하며, `syn.uicontrols.$propertypanel`의 각종 메서드에서 이 `id`(elID)를 사용합니다.
...
| 옵션명 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `data` | object \| null | `null` | 패널에 표시할 원본 값. `value`보다 우선합니다. 객체가 아니면(`null`, 배열 등) 빈 객체로 취급합니다. |
...
| `classNames` | string | `'syn-propertypanel'` | 패널 최상위 엘리먼트에 추가되는 CSS 클래스 |
...
| `'json'` | `<textarea>` | `JSON.stringify`로 표시, 저장 시 `JSON.parse`. 파싱 실패 시 `ppInvalid` 클래스가 붙고 이전 값이 유지됩니다. |
...
| 키 | 필수 | 설명 |
|---|---|---|
| `html(elemId, name, value, meta)` | 예 | 입력 엘리먼트의 HTML 문자열(또는 DOM 노드)을 반환합니다. 최상위 엘리먼트가 하나가 아니면 `span.ppCustomValue`로 감쌉니다. |
...
`syn.uicontrols.$propertypanel.<메서드명>(...)` 형태로 호출합니다.

| 메서드 | 시그니처 | 설명 |
|---|---|---|
| controlLoad | `controlLoad(elID, setting)` | 컨트롤 초기화(생성자 역할). `syn.loader.js`가 화면 스캔 시 자동으로 호출하며, 개발자가 직접 호출할 일은 없습니다. |
| render | `render(elID, value, setting)` | 주어진 `value`/`setting`으로 패널 테이블을 새로 그립니다. `setValue`가 내부적으로 사용합니다. |
| getValue | `getValue(elID, meta)` | 각 속성의 현재 입력값을 모아 `{ 속성명: 값 }` 객체로 반환합니다. |
| setValue | `setValue(elID, value, meta)` | 기존 `runtimeSetting`(또는 `setting`)을 유지한 채, 새 `value`(와 선택적으로 새 `meta`)로 패널을 다시 그립니다. |
| clear | `clear(elID, isControlLoad)` | 패널 내용(테이블 DOM과 내부 `valueFuncs`/`fields`)을 모두 비웁니다. |
| getControl | `getControl(elID)` | 등록된 컨트롤 정보(`{ id, sequence, setting, runtimeSetting, valueFuncs, fields }`)를 반환합니다. |
| createDefaultSettingMeta | `createDefaultSettingMeta(defaultSetting, meta)` | 다른 컨트롤의 `defaultSetting` 객체를 받아, 이 PropertyPanel로 "설정값 편집기"를 만들 때 쓸 `meta`를 자동 생성합니다(그룹/설명 자동 추론, `mode: 'defaultSetting'`). |
...
| setLocale | `setLocale(elID, translations, control, options)` | 다국어 처리 훅입니다. PropertyPanel는 현재 별도 구현 없이 빈 함수로 제공됩니다. |

## 이벤트 (syn-events)

PropertyPanel는 `syn-events`로 연결되는 `<elID>_<이벤트명>` 방식의 이벤트 훅을 별도로 선언하지 않습니다. 대신, 값이 바뀔 때마다 `syn-options`의 `callback` 함수가 `(element, name, value, control)` 인자로 직접 호출됩니다.
