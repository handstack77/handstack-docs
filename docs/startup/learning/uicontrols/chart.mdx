...
주의: 이 문서는 두 파일의 실제 소스 코드를 직접 읽고 확인한 내용을 바탕으로 작성되었습니다. `$chart`(Highcharts 기반)는 qcn.groupware 등에서 실사용 예시를 찾지 못해 소스 코드만으로 문서화했고, `toImage`에 실행 시 항상 예외가 발생하는 버그가 있습니다. 자세한 내용은 아래 "알려진 이슈" 절과 각 메서드 설명을 꼭 확인하세요.
...
- `$chart` (Highcharts 기반)는 Highcharts의 다양한 차트 타입(파이, 게이지, 영역 등)이나 Highcharts 고유 옵션이 꼭 필요할 때만 사용하세요. `setValue`로 시리즈를 교체하는 것은 정상 동작하지만, `toImage`(PNG 내보내기)는 현재 구현에 버그가 있어 호출 시 항상 예외가 발생합니다(아래 "알려진 이슈" 참고).
...
`/uicontrols/Chart/example/` 경로의 예제를 아래 iframe에서 바로 확인할 수 있습니다.

### chartbasic.html

<iframe
  src="/uicontrols/Chart/example/chartbasic.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="Chart chartbasic 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/Chart/example/chartjsbasic.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="Chart chartjsbasic 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/Chart/example/chartjsevents.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="Chart chartjsevents 실전 예제"
  allow="fullscreen">
</iframe>
...
<iframe
  src="/uicontrols/Chart/example/griddashboard.html"
  width="100%"
  height="640"
  style={{ border: '1px solid #ccc', borderRadius: '8px' }}
  title="Chart griddashboard 실전 예제"
  allow="fullscreen">
</iframe>
...
|---|---|
| 컨트롤 | `Chart` |
| 전역 별칭 | `syn.uicontrols.$chart` |
| 버전 | `v2026.7.22` |
| 소스 | `uicontrols/Chart/Chart.js` |
| 스타일 | `uicontrols/Chart/Chart.css` |
...
| `series` | array | 예시 2개 시리즈(`Series 1`, `Series 2`) | Highcharts 시리즈 배열(`객체 설정`) |
...
| `getValue(elID, meta)` | 각 시리즈의 `객체 설정`(`data`는 `serie.yData`) 배열을 반환합니다. 차트가 없으면 `null`. |
...
| `series` | array | `[]` | 데이터 컬럼 → 데이터셋 매핑 목록. 각 항목은 `객체 설정`. 비어 있으면 `setValue`가 `labelID`를 제외한 모든 컬럼을 컬럼명 그대로 라벨로 써서 자동으로 데이터셋을 만듭니다. |
...
| 메서드 | 설명 |
|---|---|
| `getChartControl(elID)` | 등록된 컨트롤 래퍼 객체(`객체 설정`)를 반환합니다. `chart`가 실제 Chart.js 인스턴스, `config`가 병합된 setting 객체입니다. ⚠ `$chart.getChartControl`(Highcharts 쪽)은 래퍼가 아니라 차트 인스턴스를 바로 반환하므로 두 메서드의 반환 모양이 다릅니다. |
