...
개발에 필요한 필수 환경 구성을 위한 Git, .NET Core, Node.js, curl, PowerShell 7 도구는 크로스 플랫폼 개발을 지원하므로 공식 웹 사이트에서 Windows, macOS, Linux에서 실행되는 설치 관리자를 제공합니다. 

* [git 2.44.0+ 다운로드](https://git-scm.com/downloads)
* [.NET Core 10.0 다운로드](https://dotnet.microsoft.com/ko-kr/download/dotnet/10.0)
* [Node.js v20.12.2 LTS 다운로드](https://nodejs.org/en/download)
* [Windows, Linux 및 macOS에 PowerShell 설치](https://learn.microsoft.com/ko-kr/powershell/scripting/install/install-powershell?view=powershell-7.5)
...
### Winget 을 이용한 PowerShell 설치

PowerShell을 설치하는 방법은 명령 프롬프트를 열고 다음 명령을 실행하여 최신 버전의 PowerShell 을 설치합니다.
...
> .NET Core 10.0 의 경우 macOS 12+ (Monterey) 이상을 지원합니다. macOS 11 (Big Sur) 포함 이하 버전에서는 Homebrew 와 같은 패키지 관리자에서 설치 오류가 발생하지만 [.NET Core 10.0 다운로드](https://dotnet.microsoft.com/ko-kr/download/dotnet/10.0) 에서 직접 macOS 설치 관리자를 다운로드 받아 설치 할 수 있으나 공식적으로 지원하지 않습니다.
...
### Homebrew 를 이용한 powershell 설치

```bash
brew install powershell
```

위의 명령어는 powershell 을 다운로드 하고 설치합니다.
...
위의 명령어는 .NET Core 10.0+ 을 다운로드하고, 설치를 관리자 권한으로 실행합니다.

만약 기존 dotnet 버전을 삭제 후 재설치하려면 다음 명령어로 기존 dotnet 버전을 삭제후 설치를 진행하세요.
...
### apt 를 이용한 powershell 설치

```bash
sudo apt install -y powershell
```

위의 명령어는 powershell 을 다운로드하고, 설치를 관리자 권한으로 실행합니다.
...
이 도구는 Node.js 전역 도구로서 pm2 npm 패키지에서 설치됩니다. pm2 CLI를 설치하려면 다음 명령어를 사용합니다.
...
이 도구는 Node.js 전역 도구로서 Gulp npm 패키지에서 설치됩니다. Gulp CLI를 설치하려면 다음 명령어를 사용합니다.
