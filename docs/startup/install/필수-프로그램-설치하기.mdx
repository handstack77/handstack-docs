...
다음 내용을 `windows-bootstrapper.ps1`로 인코딩을 `UTF-8 with BOM` 으로 저장한다.
...
관리자 권한으로 PowerShell에서 실행한다. 일반 사용자 권한으로 실행 하면, 시스템 범위 패키지 설치 시 WinGet이 UAC 승인을 요청할 수 있다.
...
다음 내용을 `macos-bootstrapper.sh`로 저장한다.
...
실행 권한을 부여한 뒤 일반 사용자 계정에서 실행한다. 스크립트 전체에 `sudo`를 붙이지 않는다.
...
설치 직후 현재 터미널에 프로필 설정을 다시 적용하려면 다음 명령을 실행한다.
...
다음 내용을 `ubuntu-bootstrapper.sh`로 저장한다.
...
실행 권한을 부여한 뒤 **일반 사용자 계정**에서 실행한다. 스크립트 전체에 `sudo`를 붙이지 않는다.
