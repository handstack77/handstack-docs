...
- modules 또는 modulesettings: 모듈의 동작 설정을 정의합니다.
...
HandStack 솔루션의 `1.WebHost/ack/Settings` localhost 프리셋은 실행 원본과 동일하게 관리합니다. `ack.localhost.json`은 `1.WebHost/ack/appsettings.json`, `modules/ack.<module>.localhost.json`은 각 `2.Modules/<module>/module.json`, node/syn 프리셋은 각각 `function/node.config.json`과 `wwwroot/wwwroot/syn.config.json`에 대응합니다.

서버 환경 프리셋은 대응하는 `.localhost.json`과 같은 파일 집합과 설정을 사용합니다. 단, HTTP/HTTPS 루프백 URL의 `localhost`, `127.0.0.1`, `0.0.0.0` 호스트는 환경별 기본 URL로 바꾸고 포트와 경로는 유지합니다.
...
데이터베이스 호스트처럼 웹 URL이 아닌 값은 바꾸지 않습니다. ack 런타임은 `D`, `S`, `P`만 지원하므로 test 프리셋은 `D`를 사용합니다.

```bash
handstack configuration --ack=C:/projects/handstack77/handstack/1.WebHost/build/handstack/app/ack --appsettings=ack.development.json
handstack configuration --ack=C:/projects/handstack77/handstack/1.WebHost/build/handstack/app/ack --appsettings=ack.test.json
handstack configuration --ack=C:/projects/handstack77/handstack/1.WebHost/build/handstack/app/ack --appsettings=ack.staging.json
handstack configuration --ack=C:/projects/handstack77/handstack/1.WebHost/build/handstack/app/ack --appsettings=ack.production.json
```

현재 모듈 프리셋은 `checkup`, `command`, `dbclient`, `forwarder`, `function`, `graphclient`, `logger`, `prompter`, `repository`, `transact`, `wwwroot`입니다. 솔루션에 프로젝트와 `module.json`이 없는 모듈은 모든 환경 프리셋에서 제외합니다.
