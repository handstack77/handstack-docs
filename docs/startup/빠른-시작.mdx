...
- Windows 10+: `C:/home/[사용자]/handstack`
- Linux: `/home/[사용자]/handstack`
- macOS: `/Users/[사용자]/handstack`

압축을 해제한 폴더로 이동하면 다음과 같은 디렉토리 구조를 확인할 수 있습니다.
...
cd [HandStack 소스 기본경로]
chmod +x install.sh
tr -d '\r' < install.sh > install_fixed.sh && mv install_fixed.sh install.sh # 윈도우에서 작성된 스크립트 파일을 macOS에서 실행하기 위해 줄바꿈 문자 제거합니다.
./install.sh
```

실행에 필요한 필수 설치 프로그램과 환경을 자동으로 구성합니다. 서버 사양과 네트워크 상태에 따라 10분 이상 소요될 수 있습니다.

설정이 완료되면 app 폴더로 이동하여 ack 프로그램을 실행합니다.
```bash
cd [HandStack 소스 기본경로]/app
dotnet ack.dll
```
...
cd [HandStack 소스 기본경로]
chmod +x install.sh
tr -d '\r' < install.sh > install_fixed.sh && mv install_fixed.sh install.sh # 윈도우에서 작성된 스크립트 파일을 Linux에서 실행하기 위해 줄바꿈 문자 제거합니다.
./install.sh
```
