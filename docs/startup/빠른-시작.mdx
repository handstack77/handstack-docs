...
cd [HandStack 소스 기본경로]
chmod +x install.sh
tr -d '\r' < install.sh > install.sh # 윈도우에서 작성된 스크립트 파일을 macOS에서 실행하기 위해 줄바꿈 문자 제거합니다.
./install.sh
```
...
cd [HandStack 소스 기본경로]
chmod +x install.sh
tr -d '\r' < install.sh > install.sh # 윈도우에서 작성된 스크립트 파일을 Linux에서 실행하기 위해 줄바꿈 문자 제거합니다.
./install.sh
```
