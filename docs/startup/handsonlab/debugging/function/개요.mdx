...
HandStack 서버리스 함수는 ack 프로그램에 위에 function 모듈의 기능으로 실행되는 함수로서, 다음과 같은 특징을 가지고 있습니다.

* 서버리스 함수는 컴파일 과정 없이 텍스트 파일로 만들어지며 **간단한 배포**를 통해 쉽게 배포할 수 있습니다.
* **CSharp, Node.js** 언어를 지원하며, 개수에 제한 없이 서버리스 함수를 관리할 수 있습니다.
* 서버리스 함수의 개발 및 디버깅, 운영 비용이 **줄어듭니다.**
