...
> `syn-datafield`/`syn-options` 메타 설정 없이 표준 HTMLElement나 임의의 커스텀 컨트롤을 순수 객체/배열과 가볍게 양방향 연결하고 싶다면 [syn.$bind](/docs/startup/learning/syn/binding)(`syn-bind`/`syn-bind-list` 속성 기반)를 참고하세요. 서로 다른 HTML 속성을 사용하므로 같은 화면에서 함께 사용해도 충돌하지 않습니다.
