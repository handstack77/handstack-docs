...
let $codepicker = {
    ...
    event: {
        ...
        btnToParameterString_click() {
            var parameterObject = { ApplicationID: '1', ApplicationName: 'HELLO WORLD' };
            var parameterString = syn.uicontrols.$codepicker.toParameterString(parameterObject);
            syn.$l.eventLog('btnToParameterString_click', parameterString);
        },
        ...
        btnToParameterObject_click() {
            ...
            syn.$l.eventLog('btnToParameterObject_click', JSON.stringify(parameterObject));
        },

        // 실제 시그니처: (elID, inputValue, inputText, result) - elID가 첫 인자로 온다.
        chpSubjectID_change(elID, inputValue, inputText, result) {
        }
    }
}
```