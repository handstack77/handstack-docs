...
# ASP.NET Core 10 기반 Dockerfile 가이드

ASP.NET Core 10 을 기반으로 하는 경량화된 Dockerlize 앱을 만들기 위한 Dockerfile의 구조와 각 단계에 대한 설명을 포함합니다. 해당 Dockerfile은 멀티스테이지 빌드를 활용하여 최적화된 이미지를 생성하는 방식으로 작성되었습니다.
...
- `FROM mcr.microsoft.com/dotnet/aspnet:10.0-alpine AS base`
  - **ASP.NET Core 10 런타임**을 포함한 **Alpine Linux** 기반의 가벼운 이미지를 사용합니다.
...
- `FROM mcr.microsoft.com/dotnet/sdk:10.0 AS build`
  - **.NET SDK 10.0** 이미지를 사용하여 애플리케이션을 빌드합니다.
...
## 전체 Dockerfile 및 요약
이 Dockerfile은 **멀티스테이지 빌드**를 활용하여 효율적인 ASP.NET Core 10 애플리케이션을 컨테이너화하는 방식을 설명합니다. Alpine 기반의 **경량 런타임 이미지**를 활용하여 **보안성과 성능을 높이면서도 빌드 속도를 최적화**할 수 있습니다.
