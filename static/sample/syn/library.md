...
```
내부적으로 `addEvent`/`removeEvent`는 등록된 (엘리먼트, 타입, 핸들러) 조합을 자체 레지스트리에서 관리하므로, 중복 등록을 막고 `pagehide` 시점에 일괄 해제(flush)할 수 있습니다. (모바일 브라우저의 BFCache 대응을 위해 `unload` 대신 `pagehide`를 사용합니다.)
