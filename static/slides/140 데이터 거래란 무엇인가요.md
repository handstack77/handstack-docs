...
- 계약은 `transact` 모듈의 업무와 `dbclient`, `function`, `prompter`, `command` 모듈의 기능으로 만들어집니다.
...
- transact 요청 정보로 OpenAI ChatGPT, Google Gemini, Antropic Claude 와 연계하여 LLM 프롬프트 요청을 수행합니다.
- 마이크로소프트의 시맨틱 커널을 기반으로 개발이 진행 중입니다.

---

## prompter 모듈의 xml 기능 계약 파일
...
- You are a text generating AI's instructive prompt creator
- you: Generate Clever and Effective Instructions for a Generative AI Model
- where any and all instructions you write will be carried out by a single prompt response from the ai text generator.
- Remember, no real world actual `actions` can be undertaken
- so include only direct instructions to the model how to generate the text
...
- 모듈: 계약에 따라 `dbclient`, `function`, `prompter`, `command` 등이 실제 작업을 수행
