...
프로그램 설치는 각 운영 체제의 패키지 매니저를 사용하여 명령으로 자동화된 설치 프로세스를 수행할 수 있습니다.
...
- Homebrew 를 이용한 .NET Core, Node.js 설치
...
- Linux apt-get 를 이용한 .NET Core 설치
...
- <mark>Visual Studio 2022 Community</mark> 설치하기
...
HandStack으로 만든 코드를 보고 수정하려면 <mark>코드 편집기</mark>가 필요합니다. Visual Studio 2022 Community는 오픈소스 프로젝트에서 사용 가능한 강력한 기능을 갖춘 <mark>무료</mark> 코드 편집기입니다.

- Visual Studio 다운로드를 열고 설치합니다.
- 필요한 구성 요소만 선택하여 설치 시간과 디스크 공간을 절약할 수 있습니다.
- 나중에 더 많은 구성 요소를 점진적으로 추가할 수 있습니다.
...
Visual Studio 에서 개발을 더 편하게 만들어 줄 <mark>추천 확장 프로그램</mark> 을 설치합니다. 상단 메뉴의 `확장` > `확장 관리`에서 아래 이름으로 검색하여 설치하세요.
...
## 동일한 개발 경험을 제공하는 Visual Studio Code 설치

가벼운 텍스트 에디터에서 강력한 IDE로 발전한 Visual Studio Code는 다양한 언어와 확장 프로그램을 지원합니다.
...
운영체제에 상관 없이 .NET 개발을 위한 강력한 IDE 입니다.
...
**LibMan**는 .NET Core 기반의 CLI 도구로 가벼운 클라이언트 라이브러리 획득 도구입니다.
