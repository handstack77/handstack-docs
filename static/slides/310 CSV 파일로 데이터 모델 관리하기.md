...
- LLM이 생성한 DDL SQL 실행 및 데이터베이스 테이블 확인
...
- 다음은 LLM Chat에 CSV 데이터 모델을 전달해 요청하면 생성될 수 있는 사용자 로그인 처리를 위한 `SELECT` 쿼리 예시입니다.
...
## CSV 기반 데이터 모델 관리의 장점

> CSV로 정리한 데이터 모델과 LLM Chat을 함께 활용하면, 개발자는 반복적인 SQL 작성 없이도 DB 스키마와 CRUD SQL을 빠르게 준비할 수 있습니다.

- 개발 초기 단계의 설정 시간을 대폭 단축시킵니다.
- 데이터베이스 스키마 관리를 체계적이고 간편하게 만들어줍니다.
- LLM이 생성한 쿼리를 기반으로 빠르게 기능 개발을 시작할 수 있습니다.