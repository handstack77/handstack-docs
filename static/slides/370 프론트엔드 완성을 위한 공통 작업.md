...
- `wwwroot` 모듈은 `gulpfile.js`(`scripts`, `styles`, `bundle` task)로 다수의 JS/CSS 파일을 `syn.bundle.js`, `syn.bundle.css`(및 압축된 `.min.*`)로 병합합니다.
- 배포 시점에는 `bundling` CLI(.NET 10 도구)로 대상 파일/디렉터리를 압축(`compress`, `compresspath`)하거나 `bundleconfig.json` 기준으로 일괄 처리(`artifact`)할 수 있습니다.

- 핸즈온
  - 1. 터미널에서 `dotnet run --project 4.Tool/CLI/bundling -- compresspath --path=wwwroot --keep=true` 실행
  - 2. 대상 디렉터리에 생성된 `.min.*` 파일 확인
  - 3. Chrome 개발자 도구(F12)의 Network 탭에서 로딩되는 파일들을 확인

```bash
# 저장소 루트에서 실행
dotnet run --project 4.Tool/CLI/bundling -- compresspath --path=wwwroot --keep=true
```
- 위 명령을 실행하면 대상 경로의 HTML/JS/CSS가 압축된 `.min.*` 파일로 생성됩니다.
