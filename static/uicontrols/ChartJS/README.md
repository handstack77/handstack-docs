...
- 로더: Chart.js UMD → moment.js → `chartjs-adapter-moment.js` → ChartJS (`syn.loader.js`의 `case 'chartjs'`가 순서대로 로드)
...
`scales: { x: { type: 'time' } }`(Chart.js v4 필수, npm `chartjs-adapter-moment` 미사용)는 `/lib/chart.js/chartjs-adapter-moment.js`가 번들된 `moment-with-locales.min.js`에 대해 `Chart._adapters._date.override(...)`를 직접 구현해 지원합니다. CDN이나 외부 패키지를 내려받지 않고 로컬 자산만으로 time scale까지 지원하기 위한 자체 제작 어댑터입니다.
