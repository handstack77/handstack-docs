...
var $binding = createControlBindingExample({
    adapterName: 'chart',
    initialValue: [
        {QUARTER: '1분기', AMOUNT: 20},
        {QUARTER: '2분기', AMOUNT: 28},
        {QUARTER: '3분기', AMOUNT: 32},
        {QUARTER: '4분기', AMOUNT: 40}
    ],
    get: function () {
    ...
    nextValue: function (current) {
        return current.map(function (row, index) {
            return {QUARTER: row.QUARTER, AMOUNT: row.AMOUNT + (index + 1) * 2};
        });
    },
    events: {
    ...
    business: {
        ...
        buildPayload: function (value) {
            return {transactionID: 'UD_WIDGET', input: {DashboardID: 'DASH-SALES', WidgetID: 'KPI-01', Rows: value}};
        }
    }
});
